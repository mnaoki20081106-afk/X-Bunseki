...
| `NOTIFY_MIN_INTERVAL_MINUTES` | 45 | **前回の通知から最低これだけ空ける（分）** |
| `NOTIFY_INTERVAL_OVERRIDE_SCORE` | 80 | この点数以上は上の間隔を短縮できる |
| `NOTIFY_MAX_PER_RUN` | 1 | 1回の実行で鳴らす上限 |
| `NOTIFY_MAX_PER_DAY` | 6 | 24時間の上限 |
...
- **通知が多い・うるさい** → まず `NOTIFY_MIN_INTERVAL_MINUTES` を 90 や 120 に。
  これは「最低でも何分空けるか」の指定で、スコアや件数に関係なく必ず守られます。
  それでも多ければ `NOTIFY_SCORE` を 60〜65 に。
...
| 種類 | Pushoverの優先度 | 鳴り方 |
|---|---|---|
| 通常の急上昇 | 1 (High) | おやすみモードを貫通して**1回だけ**鳴る |
| 激アツ（80点以上） | 2 (Emergency) | 確認するまで5分おき・最大10分（最大2回） |
| 学校にいる間 | 0 | おやすみモードに従う（鳴らない） |

以前は全ての通知が優先度2・60秒おき・最大1時間だったため、
**1件の通知が最大60回鳴っていました**。それが「同じ通知が何度も出る」の原因です。
