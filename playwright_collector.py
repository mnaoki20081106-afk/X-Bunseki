...
def fetch_posts() -> list[dict]:
    ...
    with sync_playwright() as p:
        ...
        # storage_state が実際に Chromium に入ったかを、値をログへ出さず確認する。
        x_cookies = context.cookies("https://x.com/")
        cookie_names = {cookie.get("name") for cookie in x_cookies}
        print(f"セッションCookie: {len(x_cookies)}件 / auth_token={'あり' if 'auth_token' in cookie_names else 'なし'} / ct0={'あり' if 'ct0' in cookie_names else 'なし'}")
        if "auth_token" not in cookie_names or "ct0" not in cookie_names:
            browser.close()
            raise SessionExpiredError(
                "storage_state に auth_token / ct0 が正しく読み込まれていません。"
            )

        # home のタイトルだけではセッション切れと判定しない。
        # GitHub Actions のIPや自動化検知でもXは「しばらくお待ちください」を返すため。
        ...
        if queries and failed_queries == len(queries):
            browser.close()
            raise RuntimeError(
                f"X検索が全{len(queries)}クエリで失敗しました。"
                "セッション切れではなく、X側のアクセス制限・画面変更・検索障害の可能性があります。"
            )
